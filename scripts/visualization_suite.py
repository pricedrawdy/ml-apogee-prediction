...
def plot_mlp_feature_importance(models: Dict, feature_names: List[str], X_test: np.ndarray):
    """
    Plot feature importance for MLP model using Gradient x Input method.
    Using 'Gradient * Input' is a common approximation for feature attribution in Neural Networks.
    """
    print("Generating MLP feature importance (Gradient * Input)...")
    
    mlp_model = models['MLP']
    input_scaler = models['input_scaler']
    
    # 1. Prepare input tensor
    # We use a subset of test data to speed up calculation if needed, but 2000 is fine
    n_samples = min(2000, X_test.shape[0])
    indices = np.random.choice(X_test.shape[0], n_samples, replace=False)
    X_sample = X_test[indices]
    X_scaled = input_scaler.transform(X_sample)
    
    # Convert to tensor and enable gradient tracking
    inputs = torch.tensor(X_scaled, dtype=torch.float32, requires_grad=True)
    
    # 2. Forward pass
    output = mlp_model(inputs)
    
    # 3. Backward pass to get d(Output)/d(Input)
    # We sum outputs to backpropagate for all samples at once
    output.sum().backward()
    
    # 4. Compute importance: mean(|Input * Gradient|)
    grads = inputs.grad.numpy()
    # Using absolute value of Input * Gradient
    impact = np.abs(inputs.detach().numpy() * grads)
    importances = np.mean(impact, axis=0)
    ...
    # Generate cleaner labels
    top_feature_names = []
    for idx_in_features in indices:
        name = feature_names[idx_in_features]
        if name.isdigit():
            idx = int(name)
            group_idx = idx // WINDOW_SIZE
            time_idx = idx % WINDOW_SIZE
            time_offset = time_idx * TIMESTEP_INTERVAL
            if group_idx < len(feature_groups):
                clean_name = f"{feature_groups[group_idx]} (t+{time_offset:.3f}s)"
            else:
                clean_name = f"Unknown Feature {idx}"
            top_feature_names.append(clean_name)
        else:
    ...
    # User requested purple bars
    bars = ax.barh(y_pos, top_importances, color='#9b59b6', edgecolor='black')
    
    ax.set_yticks(y_pos)
    ax.set_yticklabels(top_feature_names)
    ax.invert_yaxis()
    
    ax.set_xlabel('Feature Attribution (Mean |Gradient × Input|)', fontsize=12)
    ax.set_title(f'Top {top_n} Most Influential Features (MLP)', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='x')
    
    plt.tight_layout()
    save_path = OUTPUT_DIR / "mlp_feature_importance.png"
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_lr_feature_importance(models: Dict, feature_names: List[str]):
    ...
    """Plot feature importance (coefficient magnitudes) for Linear Regression model."""

    print("Generating Linear Regression feature importance...")
    ...
    importances = np.abs(lr_model.coef_.flatten())
    ...
    # User requested green bars

    bars = ax.barh(y_pos, top_importances, color='#28a745', edgecolor='black')
    ...
    ax.set_xlabel('Coefficient Magnitude (Scaled)', fontsize=12)

    ax.set_title(f'Top {top_n} Most Influential Features (Linear Regression)', fontsize=14, fontweight='bold')
    ...
    save_path = OUTPUT_DIR / "lr_feature_importance.png"
